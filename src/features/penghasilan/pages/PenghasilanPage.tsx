...
const PenghasilanPage: React.FC = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Data penghasilan berhasil ditambahkan");
      closeModal();
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Data penghasilan berhasil diperbarui");
      closeModal();
      queryClient.invalidateQueries({ queryKey: ["penghasilan"] });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Data penghasilan berhasil dihapus");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.rentang_penghasilan.trim()) return toast.warning("Rentang penghasilan wajib diisi");
      ...
      <div className="max-w-screen-2xl mx-auto space-y-8 px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-200">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Master Penghasilan</h1>
              <p className="text-sm text-slate-500 mt-1">Kelola data referensi rentang penghasilan ekonomi keluarga pendaftar.</p>
            </div>
          </div>
        ...
        <Card className="border border-slate-200 shadow-sm rounded-3xl overflow-hidden bg-white relative">
          ...
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 pb-5 pt-7 px-6 sm:px-8">
            <CardTitle className="text-xl font-bold text-slate-800">Daftar Rentang Penghasilan</CardTitle>
          </CardHeader>
          ...
          <DialogContent className="rounded-3xl border-0 shadow-2xl p-6 sm:p-8 max-w-md">
            <DialogHeader className="mb-6">
              ...
              <DialogDescription className="text-slate-500">
                Lengkapi informasi rentang penghasilan di bawah ini.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="rentang_penghasilan" className="text-sm font-bold text-slate-700 ml-1">Rentang Penghasilan</Label>
                <Input
                  ...
                  placeholder="Contoh: 1.000.000 - 2.500.000"
              ...
              <DialogFooter className="gap-2 sm:gap-3 mt-4">
                ...
                <Button 
                  type="submit" 
                  disabled={createMutation.isPending || updateMutation.isPending || !formData.rentang_penghasilan.trim()}
          ...
          <AlertDialogContent className="rounded-3xl border-0 shadow-2xl p-8 max-w-md">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-slate-600 text-base leading-relaxed mt-0">
                Apakah Anda yakin ingin menghapus data penghasilan <strong className="text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded">{deleteData?.nama}</strong>? Data yang dihapus tidak dapat dikembalikan.
              </AlertDialogDescription>
            </AlertDialogHeader>
