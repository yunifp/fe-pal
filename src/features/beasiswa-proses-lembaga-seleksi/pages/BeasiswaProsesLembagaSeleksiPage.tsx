...
const BeasiswaProsesLembagaSeleksiPage = () => {
  ...
  const { data: responseCount, isLoading: isLoadingCount } = useQuery({
    queryKey: ["count-by-provinsi-seleksi", beasiswaAktif?.id],
    queryFn: () =>
      beasiswaService.getCountByProvinsiProsesLembagaSeleksi(
        beasiswaAktif?.id ?? 0,
      ),
  ...
  // Merge data provinsi dengan jumlah pendaftar
  ...
  // Filter + sort: provinsi dengan pendaftar di atas
  ...
  const handleSelectProvinsi = (kodeProvinsi: string, namaProvinsi: string) => {
    navigate(
      `/proses_lembaga_seleksi/pendaftar/provinsi/${kodeProvinsi}?nama=${encodeURIComponent(namaProvinsi)}`,
    );
  };
  ...
  const handleDownloadRekap = async () => {
    ...
    try {
      toast.info("Mengunduh rekap data seleksi...");
  ...
  return (
    <>
      <CustBreadcrumb items={[{ name: "Proses Lembaga Seleksis" }]} />
...
export default BeasiswaProsesLembagaSeleksiPage;
