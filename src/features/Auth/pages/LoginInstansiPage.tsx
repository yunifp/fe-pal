...
const LoginInstansiPage = () => {
  ...
  const handleLogin = async (data: LoginFormData) => {
    ...
    const isValid = await authService.verifyCaptcha({
      captchaId,
    ...
    try {
      ...
      toast.error(
        error?.response?.message || error?.message || "Terjadi kesalahan",
      );
    } finally {
      setIsLoading(false);
    }
                ...
                <form onSubmit={handleSubmit(handleLogin)}>
                  <div className="grid w-full items-center gap-6">
                    ...
                    <div className="flex flex-col gap-2 text-sm">
                      {/* Pertanyaan Captcha */}
                      <div className="bg-gray-100 px-3 py-2 rounded text-center font-medium text-gray-700">
                        {captchaQuestion || "Memuat captcha..."}
                      </div>
                      ...
                      {captchaError && (
                        <Alert
                          variant="destructive"
                          className="bg-red-100 text-red-700 border border-red-400 py-1 px-2 text-xs rounded flex items-center gap-1">
                      ...
                      <Button
                        variant="link"
                        size="sm"
                        className="self-end p-0 text-blue-500 underline cursor-pointer"
