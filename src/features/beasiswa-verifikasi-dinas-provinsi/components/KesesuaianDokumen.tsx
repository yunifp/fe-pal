...
export const KesesuaianDokumen = ({
  ...
  // ✅ Logika pre-populate nilai awal
  const existingIsValid: "Y" | "N" | null =
    verifikatorMode === "dinas"
      ? (doc.verifikator_dinas_is_valid ?? null)
      : doc.verifikator_is_valid === "sesuai"
        ? "Y"
        : doc.verifikator_is_valid === "tidak sesuai"
          ? "N"
          : null;
  ...
  useEffect(() => {
    if (existingIsValid === "Y") {
      ...
      if (existingCatatan) setValue(nameCatatan, existingCatatan);
    }
  }, [existingIsValid, existingCatatan]);

  // ✅ Fungsi handle download file
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        ...
        const fileParam = urlObj.searchParams.get("file") || dokumen.file;
        const actualFile = fileParam.split('/').pop() || "";
        ext = actualFile.includes('.') ? actualFile.substring(actualFile.lastIndexOf('.')) : '.pdf';
      } catch (e) { /* fallback to pdf */ }

      const cleanTitle = (dokumen.nama_dokumen_persyaratan || "Dokumen").replace(/[^a-zA-Z0-9 \-]/g, "_");
      await downloadSecureFile(dokumen.file, `${cleanTitle}${ext}`);
    } catch (error) {
  ...
  return (
    <Controller
      ...
      render={({ field }) => {
        ...
        const bgClass = status === "Y" ? "bg-green-50 border-green-200" : status === "N" ? "bg-amber-50 border-amber-200" : "bg-white border-gray-200";

        return (
          <div className={`border rounded-lg p-4 space-y-3 transition-all ${bgClass}`}>
            <input type="hidden" {...register(`${fieldName}.${index}.id` as const)} value={dokumen.id} />
            <input type="hidden" {...register(`${fieldName}.${index}.kategori` as const)} value={fieldName === "data_persyaratan_umum" ? "Umum" : "Khusus"} />

            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <p className="font-medium text-sm">{dokumen.nama_dokumen_persyaratan || `Dokumen ${index + 1}`}</p>
                {dokumen.timestamp && (
                  <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                    <Clock className="w-3 h-3" /> Upload: {new Date(dokumen.timestamp).toLocaleString("id-ID")}
                  </p>
                )}

                {existingIsValid && (
                  <p className={`text-xs mt-1.5 flex items-center gap-1 font-medium ${existingIsValid === "Y" ? "text-green-600" : "text-amber-600"}`}>
                    {existingIsValid === "Y" ? <CheckCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                    Pernah diverifikasi: {existingIsValid === "Y" ? "Sesuai" : "Tidak Sesuai"}
              ...
              {dokumen.file && (
                <Button variant="outline" size="sm" onClick={handleDownload} disabled={isDownloading} type="button">
                  {isDownloading ? <Clock className="w-4 h-4 mr-1 animate-spin" /> : <Download className="w-4 h-4 mr-1" />}
            ...
            <RadioGroup value={field.value?.toString() ?? ""} onValueChange={field.onChange} className="flex gap-6" disabled={isReadOnly}>
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor={`${nameValid}-N`} className={isReadOnly ? "opacity-50 cursor-not-allowed" : ""}>Tidak Sesuai</Label>
              </div>
            </RadioGroup>

            {errorRadio && <p className="text-xs text-red-500">{errorRadio.message}</p>}

            {status === "Y" && (
              <div className="text-xs text-green-700 bg-green-50 p-2 rounded border border-green-200 flex items-center gap-2">
                <CheckCircle className="w-4 h-4" /> Dokumen telah diverifikasi dan sesuai
              </div>
            )}
          </div>
        );
