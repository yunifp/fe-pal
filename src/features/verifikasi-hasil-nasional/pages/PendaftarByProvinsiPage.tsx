...
const PendaftarByProvinsiPage = () => {
  ...
  const handleViewDetail = (id: number) => {
    navigate(
      `/verifikasi-hasil-nasional/pendaftar/provinsi/${kodeProvinsi}/detail/${id}` +
        `?provinsi=${encodeURIComponent(namaProvinsi)}&kodeProvinsi=${kodeProvinsi}`,
    );
  };

  const handleBack = () => navigate("/beasiswa-hasil-verifikasi-daerah");

  // ── Columns (memoised on stable deps) ────────────────────────────────────
