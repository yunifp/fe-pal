...
interface DropAndCropCircleProps {
...
const DropAndCropCircle = ({
  value,
  onChange,
  error,
  errorMessage,
}: DropAndCropCircleProps) => {
  ...
  const getCroppedImg = useCallback(async () => {
    ...
    canvas.toBlob((blob) => {
      if (blob) {
        ...
        setCroppedImageUrl(croppedUrl); // preview hasil crop

        // Reset cropper dengan gambar hasil crop yang baru:
        setImageSrc(croppedUrl); // <-- update source image agar cropper pakai gambar baru
        setCrop({ x: 0, y: 0 }); // reset posisi crop
...
export default DropAndCropCircle;
